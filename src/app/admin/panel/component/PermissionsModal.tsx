...
import { DEFAULT_ALLOWED_PAGES } from "@/lib/auth/saveUserProfile";
...
export default function PermissionsModal({
  ...
  // Default: if allowedPages is undefined/null, use ALL pages (default access)
  const initialPages: PageId[] = Array.isArray(user.allowedPages)
    ? user.allowedPages
    : (DEFAULT_ALLOWED_PAGES as PageId[]);

  const [selectedPages, setSelectedPages] = useState<PageId[]>(initialPages);
  const [isSaving, setIsSaving] = useState(false);

  const groupedPages = AVAILABLE_PAGES.reduce(
    (acc, page) => {
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center p-4">
      ...
      />
      <div
        className="relative w-full max-w-lg bg-white dark:bg-[#0d0d1a] border border-black/[0.08] dark:border-white/[0.08]
      ...
      >
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-black/[0.06] dark:border-white/[0.06] flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-gray-400 dark:text-white/30 mt-0.5 truncate">
              {user.displayName} · {selectedPages.length}/{AVAILABLE_PAGES.length}{" "}
              pages
            </p>
          </div>
          <button
        ...
        <div className="flex gap-2 px-5 pt-4 pb-2 flex-shrink-0">
          <button
            onClick={() =>
              setSelectedPages(AVAILABLE_PAGES.map((p) => p.id) as PageId[])
            }
          ...
          <button
            onClick={() =>
              setSelectedPages(DEFAULT_ALLOWED_PAGES as PageId[])
            }
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium
              bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20
              text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-500/15 transition-all"
          >
            Reset to default
          </button>
        </div>
            ...
            <div key={category}>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[10px] text-gray-300 dark:text-white/20">
                  {pages.filter((p) => selectedPages.includes(p.id as PageId)).length}/
                  {pages.length}
                </span>
              </div>
              <div className="grid grid-cols-2 gap-2">
                {pages.map((page) => {
                  const active = selectedPages.includes(page.id as PageId);
                  return (
                    <label
                      key={page.id}
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={active}
                        onChange={() => togglePage(page.id as PageId)}
                        className="sr-only"
                      />
                      <div
                      ...
                      {active ? (
                        ...
                        <Lock className="w-3 h-3 text-gray-300 dark:text-white/20 flex-shrink-0" />
                      )}
                    </label>
                  );
                })}
          ...
          {user.isAdmin && (
            <div className="flex items-start gap-2.5 px-4 py-3 rounded-xl
              ...
              <p className="text-xs text-indigo-700 dark:text-indigo-300">
                Admin users have access to all pages regardless of these
                settings.
              </p>
            </div>
          )}
