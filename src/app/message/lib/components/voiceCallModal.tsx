...
//
// Fully wired video/audio call modal.
// Receives the useWebRTC hook return value as props so the parent
// (ChatRoomPanel) owns the hook and passes it down.
//
// Usage in ChatRoomPanel:
//   const webRTC = useWebRTC({ chatId, currentUserId, currentUserName, currentUserPhoto });
//   ...
//   <VideoCallModal webRTC={webRTC} calleeName={otherUserName} calleePhoto={otherUserPhoto} />

import React, { useEffect, useState } from "react";
import {
    Mic, MicOff, Video, VideoOff, PhoneOff,
    Volume2, VolumeX, Maximize2, Minimize2, MoreHorizontal,
} from "lucide-react";
...
function CtrlButton({
    ...
    onClick: () => void; active?: boolean; danger?: boolean; label?: string; children: React.ReactNode;
}) {
    return (
...
export default function VideoCallModal({
    ...
    const {
        ...
        isMicOn, isCamOn, isSpeakerOn,
        error,
        hangUp, toggleMic, toggleCam, toggleSpeaker,
    ...
    // Close when ended
    useEffect(() => {
        if (callState === "idle" || callState === "ended") onClose();
    }, [callState, onClose]);
    ...
    return (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center" style={{ fontFamily: "'DM Sans',system-ui,sans-serif" }}>
            {/* Backdrop */}
            <div
            ...
            >
                {/* Top accent */}
                <div className="h-px w-full flex-shrink-0" style={{ background: "linear-gradient(90deg,transparent,rgba(99,102,241,0.55),transparent)" }} />

                {/* ── Video area ── */}
                {isVideo && (
                    <div className="relative flex-shrink-0 overflow-hidden bg-[#0b0b1c]" style={{ height: fullscreen ? "calc(100vh - 200px)" : 260 }}>
                        {/* Remote video */}
                        <video
                            ref={remoteVideoRef}
                            autoPlay
                            ...
                            style={{ display: isConnected && remoteStream ? "block" : "none" }}
                        />

                        {/* Remote placeholder (avatar) when not yet connected */}
                        ...
                        <div className="absolute bottom-0 left-0 right-0 h-20 pointer-events-none" style={{ background: "linear-gradient(to top,rgba(10,10,24,0.9),transparent)" }} />

                        {/* Local PiP */}
                        <div
                            className="absolute bottom-3 right-3 rounded-xl overflow-hidden"
                            style={{ width: 80, height: 108, border: "1.5px solid rgba(255,255,255,0.12)", boxShadow: "0 4px 20px rgba(0,0,0,0.6)" }}
                        >
                            {isCamOn && localStream ? (
                                <video
                                    ref={localVideoRef}
                        ...
                        {isConnected && (
                            <div
                                ...
                                style={{ background: "rgba(0,0,0,0.55)", border: "0.5px solid rgba(255,255,255,0.1)", backdropFilter: "blur(8px)" }}
                            >
                ...
                <div className={["flex flex-col items-center text-center px-6", isVideo ? "py-4" : "py-10"].join(" ")}>
                    ...
                    <p className="text-white/90 font-semibold tracking-tight" style={{ fontSize: isVideo ? 15 : 21, letterSpacing: "-0.02em" }}>
                        {calleeName}
                    </p>
                    <p className="text-[11px] text-white/35 font-medium mt-1 tracking-wide">{statusLabel()}</p>

                    {/* Audio waveform when connected (audio-only) */}
                    {!isVideo && isConnected && (
                ...
                >
                    ...
                    {/* End — largest/centered */}
                    <CtrlButton onClick={hangUp} danger label="End">
