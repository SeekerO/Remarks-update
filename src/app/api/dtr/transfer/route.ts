...
export async function POST(req: NextRequest) {
  ...
  try {
    const { rows, sheetId, sheetName, date, name, supervisor } =
      await req.json();
    ...
    const updatedDtrValues = Array.from({ length: 31 }, (_, i) => {
      ...
      const rowArray = Array.from(
        { length: 8 },
        (_, colIdx) => gridRow[colIdx]?.formattedValue || "",
      );
      ...
      const isWeekendText =
        dayTypeLabel.includes("SUNDAY") || dayTypeLabel.includes("SATURDAY");

      const hasColor = gridRow.slice(2, 6).some((cell) => {
        const bg = cell?.effectiveFormat?.backgroundColor;
        if (!bg) return false;
        const isWhite =
          (bg.red === 1 || bg.red === undefined) &&
      ...
      if (extractedRow) {
        if (isSpecialDay) {
          ...
          if (extractedRow.afternoonOut)
            rowArray[7] = extractedRow.afternoonOut;
        } else {
